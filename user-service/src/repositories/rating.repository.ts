...
export class RatingRepository {
  public async createRating(userId: number, payload: CreateRatingPayload) {
    const result = await pool.query<Rating>(
      "INSERT INTO internal_ratings (rater_id, movie_id, rating) VALUES ($1, $2, $3) RETURNING *",
  ...
  public async findRatingsByUserId(userId: number) {
    const result = await pool.query<Rating>("SELECT * FROM internal_ratings WHERE rater_id = $1", [
      userId
    ]);
    return result.rows;
  }
  ...
  public async updateRating(userId: number, payload: UpdateRatingPayload) {
    const result = await pool.query<Rating>(
      "UPDATE internal_ratings SET rating = COALESCE($1, rating) WHERE rater_id = $2 AND movie_id = $3 RETURNING *",
  ...
  public async deleteRating(userId: number, movieId: number) {
    await pool.query("DELETE FROM internal_ratings WHERE rater_id = $1 AND movie_id = $2", [
      userId,
      movieId
    ]);
  }
